...
unsigned long Scanner::put(const  string & buf){
...
const char* Scanner::TW[] = {"", "and", "bool", "do", "else","if", "false", "int", "not", "or", "program","read", "true", "while", "for", "write", "string", "struct", "goto", "break", NULL};

const char* Scanner::TD[] = {"@", "{", "}", ";", ",", "==", "(",")", "=", "<", ">", "+", "-","*", "/", "<=", "!=", ">=", ":", NULL};

 vector<Ident> Scanner::TID = {};
 ...
 ostream & operator<<( ostream &s, Lex l){
  ...
  // else if (l.t_lex == LEX_ASSIGN){
   ...
   // }
        ...
        t = Scanner::TID[l.v_lex].get_name();
...
private:
    ...
    struct Field_of_struct{
         string name;
        type_of_lex type;
        Field_of_struct( string _name, type_of_lex _type);
    };
    struct Struct{
         string name;
         vector<Field_of_struct> fields;
        Struct( string name, const  vector<Field_of_struct>& fields = {});
    };
    ...
    const char* file_name;
     ...
     stack<type_of_lex> lex_stack;
     vector<Ident> marks;
     vector<size_t> break_to_position;
     vector<size_t> marks_positions_in_poliz;

    type_of_lex get_cur_type_def() const;
    void set_type_def(const type_of_lex type);
    bool is_redefinition(Lex lex);
    bool is_redefinition( string name, const  vector<Field_of_struct>& fields);
    ...
    void m_alloc_for_struct();
     vector<Field_of_struct> definition_of_structure_fields();
    void field();
    void operators();
    void variable();
    void constant();
    ...
    type_ID F();
    bool check_types();
    void push_to_poliz( stack<size_t>& positions, unsigned long position);
...
Parser::Field_of_struct::Field_of_struct( string _name, type_of_lex _type): name(_name), type(_type) {  }

Parser::Struct::Struct( string _name, const  vector<Field_of_struct>& _fields): name(_name), fields(_fields) {  }
...
void Parser::definition_of_structures(){
 ...
 vector<Parser::Field_of_struct> Parser::definition_of_structure_fields(){
     vector<Field_of_struct> fields;
    ...
    for (Ident& elem : scan.TID) {
        if (elem.get_type() == LEX_STRUCT) {
            long long num_of_struct = elem.get_value();
             string name_struct = elem.get_name();

            for (Field_of_struct& field : structures[num_of_struct].fields) {
                 string name = name_struct + '.' + field.name;
                Ident new_id(name);
                new_id.put_declare();
    ...
    if (c_type == LEX_ASSIGN) {
        ...
        if (get_cur_type_def() == LEX_STRUCT) {
            ...
            long long num_of_struct = scan.TID[current_id].get_value();
             string name_struct = scan.TID[current_id].get_name();
...
bool Parser::is_correct_name(){
     string name = scan.TID[c_val].get_name();
    if (name.find('.') != -1) {
...
bool Parser::is_redefinition( string name, const  vector<Field_of_struct>& fields){
    ...
    if (flag)
        ...
        switch (c_type) {
            ...
            case LEX_GOTO:
                ...
                if (c_type == LEX_ID){
                    auto iter =  find(marks.begin(), marks.end(), scan.TID[c_val]);
...
void Parser::push_to_poliz( stack<size_t>& positions, unsigned long position){
...
Parser::type_ID Parser::F(){
    ...
    if (c_type == LEX_ID) {
        if(!scan.TID[c_val].get_declare()){
            auto iterator =  find(marks.begin(), marks.end(), scan.TID[c_val]);
...
void Parser::find_marks(){
    ...
    while((cur_lex = scan.get_lex()).get_type() != LEX_FIN){
        if(cur_lex.get_type() == LEX_COLON and prev_lex.get_type() == LEX_ID){
            ...
            auto iter =  find(marks.begin(), marks.end(), scan.TID[prev_lex.get_value()]);
    ...
    marks_positions_in_poliz =  vector<size_t>(marks.size());
    ...
    while (position != poliz.size()) {
        ...
        if (type_lex == LEX_ID or type_lex == LEX_NUM or type_lex == LEX_STRING_DATA or type_lex == LEX_TRUE or type_lex == LEX_FALSE or type_lex == POLIZ_LABEL or type_lex == POLIZ_ADDRESS) {
            ...
            if (type_ex == LEX_STRING) {
                 string s1 = get_string(op1);
                 string s2 = get_string(op2);
            ...
            if (type_ex == LEX_STRING){
                 cout << get_string(op) <<  endl;
            } else if (type_ex == LEX_INT) {
                 cout  << get_int(op) <<  endl;
...
long long Interpreter::get_int(Lex op){
 ...
 string Interpreter::get_string(Lex op){
     string result;
    if (op.get_type() == LEX_ID) {
...
int main(){
   ...
   Interpreter intetpreter("prog.txt");
    try {
       intetpreter.run();
